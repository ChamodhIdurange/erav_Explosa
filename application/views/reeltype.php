			...
			<div class="container-fluid mt-2 p-0 p-2">
				<div class="card">
					<div class="card-body p-0 p-2">
						<div class="row">
							<div class="col-3">
								<form action="<?php echo base_url() ?>Reeltype/Reeltypeinsertupdate" method="post"
									autocomplete="off">
									<div class="form-group mb-1">
										<label class="small font-weight-bold">Type*</label>
										<input type="text" class="form-control form-control-sm" name="reeltype"
											id="reeltype" required>
									</div>
									<div class="form-group mb-1">
										<label class="small font-weight-bold">GSM*</label>
										<select class="form-control form-control-sm" name="gsm" id="gsm" required>
...
<script>
	$(document).ready(function () {
		...
		$('#tblsuppliertype').DataTable({
			...
			ajax: {
				url: "<?php echo base_url() ?>scripts/reeltypelist.php",
			...
			"columns": [{
				...
				},
				{
					...
					"render": function (data, type, full) {
						...
						button += '" id="' + full['idtbl_reel_type'] +
							'"><i class="fas fa-pen"></i></button>';
						if (full['status'] == 1) {
							button += '<a href="<?php echo base_url() ?>Reeltype/Reeltypestatus/' +
								full['idtbl_reel_type'] +
		...
		$('#tblsuppliertype tbody').on('click', '.btnEdit', function () {
			...
			if (r == true) {
				...
				$.ajax({
					...
					url: '<?php echo base_url() ?>Reeltype/Reeltypeedit',
					success: function (result) { //alert(result);
						var obj = JSON.parse(result);
						$('#recordID').val(obj.id);
						$('#reeltype').val(obj.reeltype);
						$('#brand').val(obj.brand);
						$('#gsm').val(obj.gsm);
						$('#width').val(obj.width);
